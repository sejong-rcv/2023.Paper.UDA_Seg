...
from utils.utils import set_requires_grad, adjust_learning_rate, \
    load_state_from_model, visualize, eval_seg_atloss, kl_weight_func
from datetime import datetime

from options.print import print_options
from model.MS_UDA_attention_MANet import MS_UDA_attention_MANet
from utils.augmentation import RandomFlip_MF_fake
...
# parser.add_argument('--dataset', default='MFdataset', help='KPdataset|MFdataset')
parser.add_argument('--is_continue', action="store_true", default=False, help='restart the training')
parser.add_argument('--start_epo', default=0)
parser.add_argument('--fake', default=True, help='use fake thermal image or not')
...
if __name__ == '__main__':
    ...
    if args.dataset == 'KPdataset':
        ...
        trainloader = DataLoader(
            dataset(args.root_dir + args.data_dir, 'day', transform=augmentation_methods),
            batch_size=args.train_batch,
            shuffle=True,
            num_workers=args.num_workers,
            pin_memory=True,
    ...
    model = MS_UDA_attention_MANet(args, num_layers=args.num_layers)  # initialize model, send model to device

    log_dir = os.path.join(args.root_dir, 'log_files', model_name +'_'+ args.log_dir)
    ...
    check_dir = os.path.join(args.root_dir, 'log_files', model_name +'_'+ args.check_dir)
    if not os.path.exists(check_dir):
        os.makedirs(check_dir)

    if args.is_continue is True:
        # pass
    ...
    if args.is_continue is True:
        start_epoch = int(args.start_epo) + 1
    ...
    if args.test_mode is True:
        ...
        print('test start')
        print_dict = eval_seg_atloss(testloader, model_name, NUM_TEST_DATASET, model)
        print("{} model, epoch {} checkpoint : ".format(model_name, int(args.start_epo)))
        print(print_dict)
        print('test end')
        import pdb; pdb.set_trace()
        print('end')
    ...
    for epoch in tqdm.tqdm(range(start_epoch, args.max_epoch)):
        kl_weight = kl_weight_func(epoch, args.max_epoch)
        ...
        for i, batch in enumerate(trainloader):
            ...
            if ask_fake is True:
                ...
                if ask_dataset is True:
                    ...
                    rgb_images, th_images, gts, labels, names = batch #MF
            ...
            pred1, attentions1 = Decoder(mid_pred1)
            ...
            pred2, attentions2 = Decoder(mid_pred2)

            # pseudo_label = pseudo_label.argmax(1)
            pred2_seg_loss_G2 = loss_calc(pred2, labels)
            ...
            pred1_kl_loss_G1 = kl_loss(F.log_softmax(pred1, dim = 1), 
                                        F.softmax(pred2.detach(), dim=1))/100000
            loss_G1 = pred1_seg_loss_G1 + pred1_kl_loss_G1*kl_weight
            loss_G1.backward()

            scalar_info_G1 = {
                'G1/loss(segmentation loss)': pred1_seg_loss_G1.item(),
                'G1/loss(kl loss)': pred1_kl_loss_G1.item()}

            LOSS_G1_VALUE += loss_G1.item()

            pred2_kl_loss_G2 = kl_loss(F.log_softmax(pred2, dim = 1), 
                                        F.softmax(pred1.detach(), dim=1))/100000


            for idx,at in enumerate(attentions1): 
                attentions1[idx][0] = at[0].detach()
                attentions1[idx][1] = at[1].detach()
                  
            at_loss = 0
            for idx in range(len(attentions1)):
                at_loss += torch.sqrt(attention_loss(attentions2[idx][0],attentions1[idx][0]))
                at_loss += torch.sqrt(attention_loss(attentions2[idx][1],attentions1[idx][1]))
            at_loss = at_loss/len(attentions1)
         
            loss_G2 = pred2_seg_loss_G2 + gamma * fin_adv_loss_G2 + at_loss+ pred2_kl_loss_G2*kl_weight
            loss_G2.backward()

            scalar_info_G2 = {
                'G2/pseudo_seg_loss': pred2_seg_loss_G2.item(),
                'G2/adv_loss': fin_adv_loss_G2.item(),
                'G2/attention_loss':at_loss.item(),
                'G2/kl loss': pred2_kl_loss_G2.item()}

            LOSS_G2_VALUE_adv += fin_adv_loss_G2.item()
            LOSS_G2_VALUE_seg += pred2_seg_loss_G2.item()
            LOSS_ATTENTION_MAP += at_loss.item()
            
            #### step 3. train Decoder ####
            set_requires_grad([Decoder], True)
            for idx,p in enumerate(mid_pred1): 
                mid_pred1[idx] = p.detach()
            # detach prediction from G1 and G2

            pred1, attentions1 = Decoder(mid_pred1)
            pred1_seg_loss = loss_calc(pred1, labels)
            
            for idx,p in enumerate(mid_pred2): 
                mid_pred2[idx] = p.detach()
                
            pred2, attentions2 = Decoder(mid_pred2)
            pred2_seg_loss = loss_calc(pred2, labels)
            ...
            pred1_kl_loss = kl_loss(F.log_softmax(pred1, dim = 1), 
                                        F.softmax(pred2.detach(), dim=1))/100000
            pred2_kl_loss = kl_loss(F.log_softmax(pred2, dim = 1), 
                                        F.softmax(pred1.detach(), dim=1))/100000


            loss_decoder = alpha * pred1_seg_loss + beta * pred2_seg_loss + gamma * fin_adv_loss+alpha * pred1_kl_loss*kl_weight + beta * pred2_kl_loss *kl_weight
            ...
            scalar_info_decoder = {
                ...
                'decoder/pred1_kl_loss': pred1_kl_loss.item(),
                'decoder/pred2_kl_loss': pred2_kl_loss.item()}
            ...
            content = 'iter = {0:6d}/{1:6d}, G1 = {2:.6f} G2 = {3:.6f} G2_adv ={4:.6f} dec_pred1 = {5:.6f} ' \
                      'dec_pred2 = {6:.6f} dec_adv = {7:.6f} fin_D1 = {8:.6f} fin_D2 ={9:.6f}, att_loss={10:.6f}'.format(
                total_iters, max_iters,
                loss_G1.item(),
                pred2_seg_loss_G2.item(), fin_adv_loss_G2.item(),
                pred1_seg_loss.item(), pred2_seg_loss.item(),fin_adv_loss.item(),
                fin_adv_loss1.item(), fin_adv_loss2.item(), at_loss.item())
            #print(content)
            ...
            if i % 100 == 0 and epoch % 5==0:
                visualize(model_name, rgb_images, th_images, gts, labels, pred1, pred2, names, check_dir, epoch=epoch)
        ...
        content2 = 'epoch{0:4d}/{1:4d}, total loss for 1 epoch = G1 = {2:.6f} G2 = {3:.6f} G2_adv = {4:.6f} dec_pred1 = {5:.6f}' \
                   ' dec_pred2 = {6:.6f} dec_adv = {7:.6f} fin_D = {8:.6f}, att_loss = {9:.6f} '.format(epoch, args.max_epoch,
                                                                                    ...
                                                                                    LOSS_DECODER_VALUE_adv / len(
                                                                                        trainloader),
                                                                                    LOSS_FIN_D_VALUE / len(
                                                                                        trainloader),
                                                                                    LOSS_ATTENTION_MAP / len(
                                                                                        trainloader))
