...
def init_weights(net, init_type = 'xavier', init_gain = 1.0, net_type='decoder'):
    ...
    def init_func(m):
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            ...
            assert net_type in ['encoder_th', 'discriminator', 'encoder_rgb', 'decoder', 'recon','edge_rgb','edge_thermal','last_layer'], 'check net_type'
            if net_type in ['encoder_th', 'encoder_rgb', 'decoder', 'recon']:
                ...
                init.constant_(m.bias.data, 0.0)
            elif net_type in ['edge_rgb','edge_thermal','last_layer']:
                init.normal_(m.weight.data, std=0.001)
...
def init_net(net, init_type='normal', net_type = 'encoder_rgb', gpu = None, init_gain=1.0, pretrained=True):
    ...
    assert net_type in ['encoder_rgb', 'encoder_th', 'discriminator', 'decoder','edge_rgb','edge_thermal', 'last_layer','attention_rgb','attention_th'], 'net_type should be encoder_* | discriminator | decoder'

    # if net_type in ['discriminator', 'encoder_th']:
    if net_type in ['discriminator', 'decoder']:
        if len(gpu)>1:
    ...
    elif net_type in ['encoder_rgb', 'encoder_th']:
        # decoder
        if len(gpu)>1:
            assert(torch.cuda.is_available())
